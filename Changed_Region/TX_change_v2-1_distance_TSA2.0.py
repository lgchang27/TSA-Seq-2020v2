...
def Main():
    ...
    for line in ReadFromFile(args.bed):
        ...
        if row[0]!=chrom:
            ...
            chrom = row[0]
            bedChr.append(chrom)
            array.append((row[1],row[2],row[5])) # domain start pos, domain end pos, domain mean distance residual  
        else:
            array.append((row[1],row[2],row[5])) # domain start pos, domain end pos, domain mean distance residual 
    bedHash[chrom]=array
    print bedChr
    ...
    for i in range (len(FPKMarray1)):
        ...
        if FPKMarray1[i][1]==FPKMarray2[i][1]:
            ...
            percentile_change.append(percentileChange)
        if fpkmFoldChange > 0:
    ...
    plt.xlabel('Domain mean distance change\n'+args.output,fontsize=15)       
